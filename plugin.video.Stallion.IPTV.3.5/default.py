...
wolf = 'aHR0cDovL3Bhc3RlYmluLmNvbS9yYXcvMENrbnBLdUU='.decode('base64')
...
addon = xbmcaddon.Addon('plugin.video.Stallion-IPTV')
...
SOURCES = [{"title": "Stallion-IPTV ", "url": wolf, "fanart": "http://s19.postimg.org/smdbasv8j/fanart.jpg", "genre": "Tv Live", "date": "10.17.2015", "credits": "Stallion-IPTV", "thumbnail": "http://s19.postimg.org/4discu5gj/icon.png"}]


def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.Stallion-IPTV-%s]: %s" %(addon_version, string))
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,New source added.,5000,"+icon+")")
        if not url is None:
...
def playmediawithproxy(media_url, name, iconImage,proxyip,port, proxyuser=None, proxypass=None): #jairox
    ...
    if media_url==None or media_url=='':
        xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Unable to play empty Url,5000,"+icon+")")
        return
        ...
        try:
            while player.is_active:
                ...
                if player.urlplayed==False and time.time()-beforestart>12:
                    print 'failed!!!'
                    xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Unable to play check proxy,5000,"+icon+")")
                    break
                #xbmc.sleep(1000)
        except: pass
...
#            ln+='</item>'
        ...
        if not (regexs and 'notplayable' in regexs and not url):        
            if url:
                ...
                xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(Stallion-IPTV,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
